...
class NswDao {
    ...
    findById(id) {
        let sqlRequest = 'SELECT * ,ST_AsGeoJSON(wkb_geometry) as geom FROM nsw20180310062806 WHERE ogc_fid=${id}';
        let sqlParams = { id: id };
     ...
     */
    findAll() {
        let sqlRequest = 'SELECT  *,ST_AsGeoJSON(wkb_geometry) as geom FROM nsw20180310062806 order by ogc_fid desc fetch first 10 rows only';
    ...
    findByPaging(pageNum, pageCount) {
        let countSql = 'SELECT COUNT(*) AS count FROM nsw20180310062806';
        let pagingSql = 'SELECT  *,ST_AsGeoJSON(wkb_geometry) as geom   FROM nsw20180310062806  LIMIT ${limit} OFFSET ${offset} ';
