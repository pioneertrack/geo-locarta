...
class SaController {
        ...
        if (f=='pdf'){
            this.saDao.findById(id).then(data=>{
                ...
                */
                ...
                temp.name=temp.mark_no;
                temp.x=temp.longitude.toString().substring(0,6);
                temp.y=temp.latitude.toString().substring(0,6);
    ...
    query(req,res) { 
        ...
        this.saDao.findByPaging(pageNum,pageCount)
            .then(this.common.findSuccess(res))
            .catch(this.common.findError(res));
    }
    nearst(req,res) {
