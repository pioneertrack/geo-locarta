    ...
    findById(id) {
        ...
        return this.common.find(sqlRequest, sqlParams).then(rows =>{
            ...
            for (const row of rows) {
                sas.push(new Sa(row.ogc_fid, row.geom, row.objectid,row.longitude,row.latitude,row.height,row.mark_no,row.zone,
                    row.easting, row.northing,row.h_datum,row.h_order,row.h_fixing,row.h_purpose,row.h_authorit,row.h_adjusted,
                    row.h_position,row.h_entrydat,row.h_comment,row.v_datum,row.gs_sep,row.v_order,row.v_fixing,row.v_purpose,
                    row.v_authorit,row.v_adjusted,row.v_position,row.v_entrydat,row.v_comment,row.marktype,row.specattrib,
                    row.gone,row.wasnow,row.dupmarkno,row.symbology,row.mb_id));
            }
            return sas;
        });
    }
